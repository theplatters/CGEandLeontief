...
"""
    solve(model::Model{MobileLaborCES}; init, continuation_backoff, continuation_max_η)
...
# Continuation fallback (η ≈ 1)
When η ≥ `continuation_max_η` (default 0.98) and the Newton solver fails to
converge from the default initial guess, the solver automatically attempts a
homotopy continuation:

1. **Step down** — find a lower η where cold-start Newton converges (probes
   δ = 0.01, 0.05, 0.10, 0.20, 0.40 below the target η).
2. **Step up** — from that base η, walk up toward the target η in 0.02 steps,
   warm-starting each from the previous solution.

This addresses a structural convergence issue at η=1 (cost-minimising
allocation): the reallocation wedge penalty ½(1−η)² vanishes quadratically,
removing Jacobian curvature. For large autonomous demand shocks
(mult ≥ 0.5), the η=1 equilibrium may be unreachable by any Newton-type
solver — the function then errors with the maximum η actually reached.

Disable with `continuation_max_η=Inf` or pass an explicit `init` keyword.
"""
function solve(model::Model{MobileLaborCES};
    init = nothing,
    continuation_backoff = 0.01,
    continuation_max_η = 0.98,
)
    (; data, options, shocks) = model
    N = length(data.factor_share)
    ...
    # When η ≥ continuation_max_η (default 0.98) and the cold-start default
    # init yields a stall, attempt a homotopy continuation.
    #
    # Walk-up strategy:
    # 1. Find a lower η_base where cold-start Newton converges (probe sequence
    #    0.01, 0.05, 0.10, 0.20, 0.40 below the target).
    # 2. From η_base, walk up toward η_target in 0.02 steps, warm-starting
    #    each from the previous solution. 0.02 is the largest reliable step
    #    — empirically, steps beyond ~0.02 stall above η≈0.94 for large
    #    autonomous shocks (autonomous_mult ≥ 0.5).
    #
    # If the target η is unreachable (the reallocation wedge vanishes,
    # removing Jacobian curvature), the function errors with a clear
    # message reporting the maximum η actually reached.
    η_continuation_reached = -Inf
    if string(res.retcode) != "Success" && !user_init && η ≥ continuation_max_η
        @info "MobileLaborCES.solve: cold-start stalled at η=$η; " *
              "attempting continuation (walk up in 0.02 steps)"
        for δ in (0.01, 0.05, 0.10, 0.20, 0.40)
            η_base = max(η - δ, 0.0)
            η_base > continuation_max_η && η_base == η && continue
            base_el = MobileLaborCESElasticities(
                options.elasticities.θ, options.elasticities.ϵ,
                options.elasticities.σ, η_base,
            )
            base_model = Model(data, shocks,
                MobileLaborCES(base_el, options.labor_bar, options.closure),
            )
            res_base = NonlinearSolve.solve(
                NonlinearSolve.NonlinearProblem(problem, init, base_model),
                reltol=1e-6, abstol=1e-6, maxiters=1000,
            )
            string(res_base.retcode) == "Success" || continue
            # Walk up from η_base toward η_target in increments of 0.02.
            # Use integer hundredths to avoid floating-point drift.
            cur_hundredths = round(Int, η_base * 100)
            step_hundredths = 2
            target_hundredths = round(Int, η * 100)
            x_cur = res_base.u
            while cur_hundredths < target_hundredths
                next_hundredths = min(cur_hundredths + step_hundredths, target_hundredths)
                η_next = next_hundredths / 100.0
                step_el = MobileLaborCESElasticities(
                    options.elasticities.θ, options.elasticities.ϵ,
                    options.elasticities.σ, η_next,
                )
                step_model = Model(data, shocks,
                    MobileLaborCES(step_el, options.labor_bar, options.closure),
                )
                res_step = NonlinearSolve.solve(
                    NonlinearSolve.NonlinearProblem(problem, x_cur, step_model),
                    reltol=1e-6, abstol=1e-6, maxiters=1000,
                )
                if string(res_step.retcode) != "Success"
                    η_reached = cur_hundredths / 100.0
                    η_continuation_reached = max(η_continuation_reached, η_reached)
                    @warn "walk-up stalled at η=$(η_reached) → η=$(η_next)"
                    break
                end
                cur_hundredths = next_hundredths
                x_cur = res_step.u
            end
            if cur_hundredths == target_hundredths
                η_final = cur_hundredths / 100.0
                res = NonlinearSolve.solve(
                    NonlinearSolve.NonlinearProblem(problem, x_cur, model),
                    reltol=1e-6, abstol=1e-6, maxiters=1000,
                )
                if string(res.retcode) == "Success"
                    @info "MobileLaborCES.solve: η=$η converged via " *
                          "η=$(η_base) → ... → η=$η continuation ✓"
                    break
                else
                    η_continuation_reached = max(η_continuation_reached, η_final)
                    @warn "final Newton step at η=$η stalled after walk-up"
                end
            end
        end
    end

    if string(res.retcode) != "Success" && η_continuation_reached > -Inf
        η_best = round(η_continuation_reached, digits=2)
        error("MobileLaborCES.solve failed at η=$η; continuation reached " *
              "η=$η_best. The η=1 equilibrium is structurally unsolvable " *
              "for this shock magnitude because the reallocation wedge " *
              "penalty ½(1−η)² vanishes, removing Jacobian curvature. " *
              "Try a smaller autonomous/mult or use η=$η_best directly.")
    end
    string(res.retcode) == "Success" ||
