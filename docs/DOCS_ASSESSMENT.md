...
\textcolor{revisionV7}{Version 8 records the executed supply arm (ADR-0023): the plain channels the equivalence analysis left as an open route are now measured. Three designs --- \texttt{supply\_etas\_s1} (sector-1 shock, $A_1$ = 1.1/1.2/1.3), \texttt{supply\_etas\_prog} (the programme's own sectors, $A_i$ = 1 + $\alpha\psi_i$, $\alpha$ = 0.05/0.10/0.20), \texttt{supply\_etas\_unif} (uniform, $A_i$ = 1.01/1.02/1.03) --- 63 cells each, every cell executed with all gates passing. The identification result is exact and magnitude-invariant (implied $\ln L / \ln w$ = input $\eta_s$ to 1e-3, spread across the magnitude ladders below 1e-3); the ALPHA control pins $L$ = 1 (w = 1.0049 / 1.0046 / 1.0408); financing neutrality F2 = F3 holds under every supply shock; and the ADR-0019 external-account canary is asserted at $A \neq 1$ on all 189 cells --- the ETAs gate that had only been tested at $p$ = 1. Finding 2's supply-side parenthetical is thereby promoted from probe to executed generation (the upgrade note sits under finding 2). The programme size is settled at the deflated horizon mean (ADR-0024: G0 = 40.3 bn at 2019 prices = 1.331 \% of GDP, i.e. the 2023-price mean 58.9 bn / 1.46 deflator). The revision palette restarts with the brighter cycle for Version 8 (revisionV7..V12, the same hues at higher brightness).}
...
\textcolor{revisionV7}{\textbf{Finding 2 is now a measured identification result, not a probe.} The supply arm (ADR-0023) is executed: \texttt{supply\_etas\_s1} ($A_1$ = 1.1/1.2/1.3 on sector 1), \texttt{supply\_etas\_prog} ($A_i$ = 1 + $\alpha \psi_i$ on the programme's own sectors, $\alpha$ = 0.05/0.10/0.20) and \texttt{supply\_etas\_unif} ($A_i$ = 1.01/1.02/1.03 uniform), 63 cells each on the identification matrix ALPHA control + scalar single-wage BETA ($\eta_s$ = 0.5/1/2/5) + sectoral uniform BETA ($\eta_s$ = 0.5/2), times F1/F2/F3. All 189 cells execute with every gate passing. The implied elasticity $\ln L / \ln w$ recovers the input $\eta_s$ exactly (0.5000/1.0000/2.0000/5.0000) in each scalar BETA cell and is invariant to the shock magnitude (spread $< 10^{-3}$); the ALPHA control pins $L$ = 1 with $w$ = 1.0049 (s1) / 1.0046 (prog) / 1.0408 (unif, $A$ = 1.02); financing neutrality F2 = F3 holds under every supply shock; and the ADR-0019 external-account canary is asserted at $A \neq 1$ on all 189 cells --- the gate that had only ever been tested at $p$ = 1. The recombination note's $(\eta, \eta_s)$ 2 x 2 is thereby realized in its informative regime: the supply arm separates the allocation margin (composition) from the supply elasticity (the level), exactly as the note anticipated. Flow tables: \texttt{paper/tables/supply\_etas\_\{s1,prog,unif\}\_flows.md}; workplan and signatures: \texttt{docs/ETAs.md} (v5).}
...
- **Version 8** \textcolor{revisionV7}{(September 2026)} --- The supply arm is
  executed (ADR-0023): 189 cells across \texttt{supply\_etas\_s1/prog/unif},
  all gates pass. $\eta_s$ is now identified under supply shocks (implied
  $\ln L / \ln w$ = input to 1e-3, magnitude-invariant); the ALPHA control
  pins $L$ = 1; F2 = F3 financing neutrality holds under supply shocks; the
  external-account canary is asserted at $A \neq 1$ for the first time.
