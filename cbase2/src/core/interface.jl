...
`src/calibration.jl`) rebuilds the IO table without the dropped sectors and
passes the reduced count, so `Ω_raw`, `λ`, `labor_share` and the GDP aggregates
are recomputed on the retained economy: every `Ω_raw` row is normalized on the
retained suppliers and Σ `labor_share` is exactly 1, instead of inheriting
full-table shares (the `drop_sectors` defect).

Returns a NamedTuple; `assemble_data` binds it to the table in a `Data` object.
"""
...
`src/calibration.jl`; the compact synthetic constructor in this file is
unaffected. Shared by `read_data` (full table) and `retained_dataset`
(rebuilt retained-sector table).
"""
function assemble_data(io::DataFrames.DataFrame, d::NamedTuple)
	...
	return Data(io, d.Ω, d.Ω_raw, d.consumption_share, d.factor_share, d.λ,
			...
			d.import_share, d.domestic_final_demand, zeros(n), zeros(n), zeros(n),
			zeros(n), zeros(n), 0.0, d.gdp_production, d.gdp_income, d.gdp_expenditure)
end
