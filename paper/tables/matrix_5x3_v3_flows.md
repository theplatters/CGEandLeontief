# Flow tables -- the 5x3 matrix on the A-bill calibration (ADR-0015 generation)
...
`Real GDP rel.` is the Tornqvist index against the baseline (baseline = 1); `L` is
total employment (`L` = 1); `Net ext. position` is the model's own
...
| Cell | Real GDP rel. | L | S | tax | I + X | M | T | Net ext. position |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
...
| F2 | programme financed by a lump-sum tax (`tau0` 0.2141 -> 0.2274) | -0.000601 | -0.000002 |
...
content is 0.002851; the *net* external position is -0.008495 for the mobile eta = 1
...
- Employment is pinned at `L = 1` in the mobile rows (ALPHA/BETA) and endogenous
