...
%
% Repo: BFRep/(3)BeyondHulten, branch `reorg`, base commit ac3d12a (2026-09-19).
% Cited evidence: the matrix_5x3-v6-* runs (ADR-0019 + ADR-0020 option C
% generation, 15/15 executed 2026-09-18, commit 6db1da5) and
% paper/tables/matrix_5x3_v6_flows.md; the cbase2 analytic equivalence test
...
% the matrix_5x3_v9 generation (Table tab:v9). The abstract, the section
...
price channel (ADR-0020).} The final
...
\texttt{matrix\_5x3\_v6} generation (ADR-0019 \textcolor{revisionV1}{together
with the ADR-0020 option C endpoint}; 15/15 cells, run ids
\texttt{matrix\_5x3-v6-*}) measures two coincidences:
...
$4.4\times10^{-16}$ to $1.8\times10^{-13}$ across the six fixed-wage cells,
\texttt{runs/matrix\_5x3-v6-\{GAMMA,DELTA\}-*/manifest.toml}). The
...
in \texttt{docs/DOCS\_ASSESSMENT.md} section 4.2), and the v6 flow metrics
...
\paragraph{Measured.} Table~\ref{tab:v9} reports the executed sectoral cells
(\texttt{matrix\_5x3-v9-BETA-*}; ADR-0022). Four readings.
...
\begin{table}[htbp]
\centering
\caption{The sectoral family in the \texttt{matrix\_5x3\_v9} generation
(\texttt{matrix\_5x3-v9-BETA-*}; deviations from the no-programme baseline;
...
from a cold start --- as Proposition~\ref{prop:nest} requires. No separate
...
(\texttt{probe15\_sectoral\_supply.jl}). The same probe shows why $\eta_{s,i}$
...
\textcolor{revisionV1}{Three scalar regimes occur across the $\eta = 1$
rows, and the $\eta = 0$ endpoint is now a fourth, genuinely nonlinear
...
decimals in every mobile row (Table~\ref{tab:v6}).
...
\paragraph{Admissibility.} All fifteen cells execute under the
\textcolor{revisionV1}{v6} calibration. In particular the F1 column is
...
sectoral family (ADR-0022): every headline metric and diagnostic of the fifteen
cells reproduces the v6 values bit-for-bit ($\max|\Delta| = 0$ on
\texttt{gdp\_rel}, \texttt{consumption\_rel}, employment, the deflator,
$\max|p-1|$, the external transfer, the wage extremes and the identity
residual), so the equivalences and the table below are invariant to the
extension. The sectoral cells of the same generation are reported separately in
Table~\ref{tab:v9}.}

Table~\ref{tab:v6} extracts the rows of the executed
\texttt{matrix\_5x3\_v6} generation (income-side real GDP and the
...
\texttt{paper/tables/matrix\_5x3\_v6\_flows.md}). \textcolor{revisionV1}{The
...
\begin{table}[htbp]
\centering
\caption{The equivalence rows in the \texttt{matrix\_5x3\_v6} generation
(run ids \texttt{matrix\_5x3-v6-*}; deviations from the no-programme
baseline).}
\label{tab:v6}
\begin{tabular}{llrrr}
\toprule
...
inside a flexible-wage closure, and it leaves the executed matrix bit-for-bit
unchanged. The manuscript can therefore state three exact equivalences and
...
generation (Table~\ref{tab:v9}). The abstract, the section roadmap, the
...
not (design heterogeneity, not demand-sensitivity). No proposition or proof
changed.
