...
function elasticity_gradient(shocks)
	...
	elasticities = CESElasticities(0.01, 0.5, 0.95)
	ces = CES(elasticities)
	model = Model(data, shocks, ces)
	sol = solve(model)
	for i in range(0.98, 0.015, 100)
...
p3 = plot(first.(a1), last.(a1), label = "Elasticty between goods", title = "Real GDP")
plot!(first.(b1), last.(b1), label = "Elasticty between labour and goods")
plot!(first.(c1), last.(c1), label = "Consumption elasticity")
plot!([0.99, 0.015], fill(gdp_effect_simple, 2), label = "Proportion of shock to GDP")
plot!([0.99, 0.015], fill(gdp(sol_leontief,model_leontief),2), label = "Calculated GDP of the Leontief Model")
...
Plots.plot(p3,p4, size=(1980,1080))
Plots.savefig("plots/elasticity_gradient_50000.html")
