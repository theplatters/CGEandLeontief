function [ NewOmega, Elasticities, FinalDemand] = Relabel_HT_adj_costsI(Omega, alpha,epsilon,theta,beta, N, M, sigma)
% N -- number of sectors
% M -- number of inputs
...
%%
NewOmega = zeros(3*N+M+2, 3*N+M+2);

for i=1:N
   NewOmega(i+2,N+i+2) = 1-alpha(i, 1); % original industries use their corresponding intermediate inputs...
   NewOmega(i+2,3*N+i+2) = alpha(i, 1); %... and their corresponding value added

end
NewOmega(N+1+2:2*N+2, 2*N+1+2:3*N+2) = eye(N);
NewOmega(2*N+1+2:3*N+2, 1+2:N+2) = Omega;
%%
Elasticities=[1; sigma; epsilon;ones(M, 1); theta; ones(M, 1)]; % Elasticities of substitution for transformed model
